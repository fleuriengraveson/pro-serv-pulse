...
import { CATEGORIES, DEFAULT_TIER_MAP, TIERS, TIME_DEFAULTS } from './config.js';
...
function attachSettingsListeners() {
  ...
  const saveFields = async () => {
    ...
    settings.name = container.querySelector('#setting-name')?.value.trim() || '';
    settings.role = container.querySelector('#setting-role')?.value || 'contributor';
    settings.dayStartHour = parseInt(container.querySelector('#setting-start-hour')?.value) || TIME_DEFAULTS.dayStartHour;
    settings.dayEndHour = parseInt(container.querySelector('#setting-end-hour')?.value) || TIME_DEFAULTS.dayEndHour;
    settings.lunchStartHour = parseInt(container.querySelector('#setting-lunch-hour')?.value) || TIME_DEFAULTS.lunchStartHour;
    settings.lunchBlocks = parseInt(container.querySelector('#setting-lunch-blocks')?.value) || TIME_DEFAULTS.lunchBlocks;
    settings.enableMerchant = container.querySelector('#setting-merchant')?.checked || false;
    settings.enableFormerPOS = container.querySelector('#setting-formerpos')?.checked || false;
  ...
  container.querySelector('#setting-role')?.addEventListener('change', async () => {
    await saveFields();
  ...
  container.querySelectorAll('.tier-select').forEach(select => {
    select.addEventListener('change', async () => {
      const tierMap = await getTierMap();
      tierMap[select.dataset.category] = parseInt(select.value);
  ...
  container.querySelector('#btn-reset-tiers')?.addEventListener('click', async () => {
  ...
  /* --- Restore from backup --- */
  container.querySelector('#btn-restore-backup')?.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    if (!file) return;

    /* Confirm before overwriting all data */
    if (!confirm('This will replace ALL your local data with the backup file. Are you sure?')) {
      e.target.value = '';
      return;
    }
